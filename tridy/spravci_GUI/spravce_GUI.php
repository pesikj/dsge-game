...
class spravce_GUI extends Trida_reagujici_na_udalosti {
    ...
    public function zpracuj_zadost_o_uzivatelske_rozhrani($odesilatel, $parametry) {
        ...
        echo "<div id=\"wrap\">";
            ...
            if ( ($_SESSION['auth'] != 1) && (getenv("REMOTE_USER") == false) ) {
                ...
                $hrac = $GLOBALS['hrac'];
                $this->hrac = $hrac;
                if ($hrac->get_over_zapojeni_hrace_do_hry() == true) {
    ...
    private function vygeneruj_a_vloz_zadane_uzivatelske_rozhrani($parametry) {
        $predchozi_ucast_ve_hre = $this->hrac->get_over_zapojeni_hrace_do_hry();
        if ($predchozi_ucast_ve_hre == false) {
            $formular_registrace = new Formular_registrace();
            $formular_registrace->generuj_formular_zapojeni_do_hry();
            return;
        }
        ...
        } else if ($parametry['id_stranky'] == 'export_dat') {
            $this->vloz_obsah_stranky_export_dat();
        }
    }
    ...
    private function formular_trhu_zajisteni_dat_a_vygenerovani($nazev_tabulky, $oznaceni_P) {
        ...
        } else if (strstr($nazev_tabulky, 'poptavka')) {
    ...
    private function vloz_obsah_stranky_prehled() {
        ...
        if ($this->cislo_aktualniho_kola > 1) {
            ...
            echo "<p>Transakce provedené v minulém kole <i>jedná se o pomocné hodnoty zadané administrátorem</i></p>";
            echo "<ul><li>nakoupili jste 7 kusů spotřebního zboží a prodali 7 kusů spotřebního zboží</li>";
            echo "<li>nakoupili jste 1 kus kapitálového zboží a prodali 1 kus kapitálového zboží</li>";
            echo "<li>nakoupili jste 5 hodin práce a prodali 5 hodin práce</li></ul>";
        }

        echo "<img src=\"funkce.png\" alt=\"Hodnotící funkce\"/>";
    }
    ...
    private function vloz_obsah_stranky_export_dat() {
        $formular_exportu_dat = new Formular_exportu_dat();
        $formular_exportu_dat->vloz_formular_exportu_dat();
